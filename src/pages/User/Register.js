  ...
  handleSubmit = e => {
    ...
    form.validateFields({ force: true }, (err, values) => {
      if (!err) {
        ...
        $.ajax({
          ...
          success: data => {
            ...
            if (data.code == true) {
              notification.open({
                message: '成功',
                description: '注册成功,需要管理员审批通过后才可以使用该账户！',
              });
              var sp1 = window.location.href.split("/user"), sp2 = window.location.href.split("/User");
              if (sp1.length > 1) {
                window.setTimeout(() => {
                  window.location.href = sp1[0] + "/login";
                },2000); 
                return;
              }
              if (sp2.length > 1) {
                window.setTimeout(() => {
                  window.location.href = sp2[0] + "/login";
                },2000); 
                return;
              }
        ...
        notification.error({
          message: "错误", 
          description: "用户名、邮箱格式错误或者密码过于简单！"
        });
      }
    });
